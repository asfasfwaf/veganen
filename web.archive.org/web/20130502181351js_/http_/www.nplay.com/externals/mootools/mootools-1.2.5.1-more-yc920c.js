...
return b;},parseDirectory:function(b,c){b=(b.substr(0,1)=="/"?"":(c||"/"))+b;if(!b.test(URI.regs.directoryDot)){return b;}var a=[];b.replace(URI.regs.endSlash,"").split("https://web.archive.org/").each(function(d){if(d==".."&&a.length>0){a.pop();
}else{if(d!="."){a.push(d);}}});return a.join("https://web.archive.org/")+"/";},combine:function(a){return a.value||a.scheme+"://"+(a.user?a.user+(a.password?":"+a.password:"")+"@":"")+(a.host||"")+(a.port&&a.port!=this.schemes[a.scheme]?":"+a.port:"")+(a.directory||"/")+(a.file||"")+(a.query?"?"+a.query:"")+(a.fragment?"#"+a.fragment:"");
...
}var a=g.file+(g.query?"?"+g.query:"")+(g.fragment?"#"+g.fragment:"");if(!f.directory){return(g.directory||(g.file?"":"./"))+a;}var d=f.directory.split("https://web.archive.org/"),c=g.directory.split("https://web.archive.org/"),h="",j;
var b=0;for(j=0;j<d.length&&j<c.length&&d[j]==c[j];j++){}for(b=0;b<d.length-j-1;b++){h+="https://web.archive.org/web/20130530032451/http://";}for(b=j;b<c.length-1;b++){h+=c[b]+"/";}return(h||(g.file?"":"./"))+a;
...
k[q]=this.container.getStyle("margin-"+q).toInt();m[q]=d?d.getStyle("padding-"+q).toInt():0;g[q]=d?d.getStyle("border-"+q).toInt():0;},this);var f=this.element.offsetWidth+c.left+c.right,p=this.element.offsetHeight+c.top+c.bottom,j=0,l=0,o=i.right-h.right-f,a=i.bottom-h.bottom-p;
...
/*
     FILE ARCHIVED ON 18:13:51 May 02, 2013 AND RETRIEVED FROM THE
...
playback timings (ms):
  ...
  esindex: 0.011
  cdx.remote: 19.769
  LoadShardBlock: 220.387 (3)
  PetaboxLoader3.datanode: 111.037 (4)
  PetaboxLoader3.resolve: 258.032 (2)
  load_resource: 185.395
*/