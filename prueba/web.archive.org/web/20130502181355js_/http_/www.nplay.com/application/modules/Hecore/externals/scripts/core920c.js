...
var HEContacts = new Class({
  ...
  init: function() {
    ...
    if (this.$listAll) {
      this.$listAll.addEvent('click', function(){
        self.chooseList('all');
      });
    }

    if (this.$listSelected) {
      this.$listSelected.addEvent('click', function(){
        self.chooseList('selected');
      });
    }

    if (this.$filterSubmit) {
      this.$filterSubmit.addEvent('click', function(){
        self.search();
      });
    }
    ...
    var self = this;
    this.$filter.addEvent('keyup', function(event) {
      if (event.code == 13) {
        self.search();
      }
    });
...
var he_list = {
  ...
  box: function(module, list, title, params){
    ...
    Smoothbox.open($el, {width: 535, height:415});
    //Smoothbox.open($el);
  },
...
/*
     FILE ARCHIVED ON 18:13:55 May 02, 2013 AND RETRIEVED FROM THE
...
playback timings (ms):
  ...
  esindex: 0.012
  cdx.remote: 21.707
  LoadShardBlock: 309.079 (3)
  PetaboxLoader3.datanode: 222.545 (4)
  PetaboxLoader3.resolve: 277.768 (3)
  load_resource: 199.813
*/